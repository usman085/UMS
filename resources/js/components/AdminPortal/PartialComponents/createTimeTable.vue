<template> 
  <div class="create-time-table-wrapper">
    <v-card>
      <v-card-title class="text-center time-header"><b class="tmeTableHead">Daily Time Table</b></v-card-title>
      <!-- Card Sub title On Inserted Object -->
      <v-card-subtitle>
        <span v-if="!scheduleHead.program == ''">       
        Program : {{ scheduleHead.program | capitalize }} 
        </span>
        ...
        <span v-if="!scheduleHead.section == ''">
        <v-icon>mdi-chevron-double-right</v-icon>
        Section : {{scheduleHead.section | capitalize}} 
        </span> 
          ...
          <template>
            <thead>
              <tr>
                <th colspan="4">
                  <span>Lectures</span>
                <v-btn @click="$store.dispatch('CreateTimeTableModal')" class="add-more-btn">Add More Lectures</v-btn></th>
              </tr>
            </thead>
            <tbody>
              <!-- Monday Rendering -->
              <tr>
                ...
                <td v-for="(timeTable,index) in mondaySchedule" :key="index" >
                  ...
                  <span class="action-icons"><v-icon @click="editEntry(timeTable.id)">mdi-pencil</v-icon> <v-icon @click="delEntry(timeTable.id)">mdi-delete</v-icon> </span>
                </td>    
              </tr> 
              <!-- Monday Rendering -->
...
export default {
    ...
    components:{TimeTableModal,timeTableDetail}, //Register Components
    // Mounted Hook
    mounted(){
      // *** Update Array
      ...
      EventBus.$on('timeTableDetail',(data)=>{
          ...
          this.scheduleHead.section=data.section;
