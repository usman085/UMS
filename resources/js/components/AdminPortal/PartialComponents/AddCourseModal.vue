        ...
        <v-card>
            ...
            <v-container>
              <v-form v-model="valid" ref="form">
                <v-row>
                  <v-col cols="12">
                    <v-text-field
                      v-model="courseDetail.course_code"
                      :rules="FieldRules"
                      required
                      label="Course Code"
                      hint="SWE-403"
                  ...
                  <v-col cols="12">
                    <v-text-field
                      v-model="courseDetail.course_title"
                      :rules="FieldRules"
                      label="Course Title*"
                      hint="Web Engineering"
                      required
                    ></v-text-field>
                  </v-col>
                  ...
                  <v-col cols="12">
                    <v-text-field
                      v-model="courseDetail.credit_hours"
                      :rules="FieldRules"
                      required
                      label="Credit Hours"
                      hint="4(3-1)"
          ...
          <v-card-actions>
            ...
            <v-btn
              small
              color="error"
              @click="$store.dispatch('AddCourseModalToggle');$refs.form.reset();"
            >Close</v-btn>
            <v-btn
              small
              :disabled="!valid"
...
export default {
  ...
  methods: {
    ...
    editCourse: function() {
      ...
      axios
        .post(process.env.MIX_APP_URL + "/edit-course", 
        this.courseDetail, {
          headers: headers
        })
        .then(res => {
