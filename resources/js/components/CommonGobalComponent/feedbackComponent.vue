...
<div class="feed-back-form-wrapper">
    <v-card class="mx-auto" max-width="100%">
        ...
        <div class="form-rating">
            <div class="rating">
                <v-alert type="error" v-if="error">Select Rating Stars</v-alert>
                <span class="rating-head">
                    How was your experiance?
                    <div class="rating-star">
                        <v-rating v-model="rating" :length="5" color="#0091ea" background-color="grey lighten-1"></v-rating>
                        <div>
                            <span class="caption text-uppercase">Stars:</span>
        ...
        <v-form v-model="valid" ref="form">
            <div class="descption-box-wrapper">
                <v-container>
                    <v-row>
                        <v-col cols="12" sm="12">
                            <v-text-field label="Your Name" required :rules="nameRules" @blur="checkWords(name)" v-model="name"></v-text-field>
                        </v-col>
                    </v-row>
                </v-container>
                <div class="descption-content">
                    <span class="descripton-head">Describe your experience here.</span>
                    <ckeditor @blur="checkWords(editorData)" :editor="editor" v-model="editorData" :rules="nameRules" required :config="editorConfig"></ckeditor>
                </div>
                <!--descption-content-->
            </div>
